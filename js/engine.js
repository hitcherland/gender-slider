...
function VectorOperation(operation,v1,v2) {
	var len=Math.min(v1.length,v2.length)
	var output=[]
	for(var i=0;i<len;i++) output[i]=operation(v1[i],v2[i])
	return output
}

function VectorAdd(v1,v2) { return VectorOperation(function(a,b) { return a+b; },v1,v2); }
function VectorSub(v1,v2) { return VectorOperation(function(a,b) { return a-b; },v1,v2); }
function VectorMul(v1,v2) { return VectorOperation(function(a,b) { return a*b; },v1,v2); }
function VectorDiv(v1,v2) { return VectorOperation(function(a,b) { return a/b; },v1,v2); }
...
class Mesh extends GameObject {
	...
	set scale(s) {
		...
		var relative=VectorDiv(s,this.__scale)
		this.__scale=s;

		if(relative[0]==1&&relative[1]==1&&relative[2]==1) return;
		...
		for(var i=0; i<this.children.length; i++) {
			this.children[i].scale=VectorMul(this.children[i].scale,relative);
		}
	}
    ...
    gl.vertexAttribPointer(shaderProgram.textureCoordAttribute, this.textureBuffer.itemSize, gl.FLOAT, false, 0, 0);
		...
		if(this.texture===undefined ||this.texture.useLighting ) {
			gl.uniform1i(shaderProgram.useLighting,1);
		} else if (!this.texture.useLighting) {
			gl.uniform1i(shaderProgram.useLighting,0);
		}
...
class Game extends GameObject {
	...
	initGL() {
		gl = this.canvas.getContext('webgl2') || this.canvas.getContext('experimental-webgl');
	...
	draw() {
		...
		mat4.ortho(-this.canvas.width/2, this.canvas.width/2, -this.canvas.height/2, this.canvas.height/2, 0.1, this.drawDistance,pMatrix);
		...
		gl.uniform1f(shaderProgram.transparency, 0.5);
	}
